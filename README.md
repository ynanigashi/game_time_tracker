...
- **堅牢なエラーハンドリング**: 設定ファイルの検証、スプレッドシート書き込み失敗時の適切な処理。
...
- モード・位置・サイズは `window_state.txt` に保存/復元されます。
...
### 設定ファイルのエラー
- `config.ini` の必須項目（`[LOGHANDLER]` の `json_file_path`, `sheet_key` 、`[GAMEINFO]` の `sheet_key`, `sheet_gid`）が欠けていると起動時にエラーになります。
