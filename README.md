...
- `レポート` の `ログ` タブではプレイログ一覧を確認でき、`スプシ同期` からプレイログシートを手動同期できます。スプレッドシート側のログを取り込み、未バックアップのローカルログを送信します。同期後は取得件数、取込件数、取込スキップ件数、未送信件数、バックアップ件数、失敗件数、上書き/別ID採番件数、エラー原因をステータスに表示します。
